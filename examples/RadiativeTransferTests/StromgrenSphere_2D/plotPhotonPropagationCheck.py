...
def plot_photons(filename, emin, emax, fmin, fmax):
    ...
    if use_const_emission_rates:
        ...
        if mask.any():
            ...
            ax1.plot(rA, EA / line(rA.v, lin_res.slope, lin_res.intercept), **lineplot_kwargs, linestyle="--", c="k", label="analytical solution")
    else:
    ...
    if use_const_emission_rates:
        ...
        ax2.plot(rA, EA.to(energies.units), **lineplot_kwargs, linestyle="--", c="k", label="analytical solution")
    else:
        # just plot where photon front should be
    ...
    ax3.plot([r_expect, r_expect], [emin_to_use, emax*1.1], label="expected photon front", color="red")
    ...
    if use_const_emission_rates:
        ...
        if mask.any():
            ...
            ax3.plot(rA, FA / line(rA.v, lin_res.slope, lin_res.intercept), **lineplot_kwargs, linestyle="--", c="k", label="analytical solution")
    else:
        # just plot where photon front should be
        ax1.plot([r_expect, r_expect], [emin_to_use, emax*1.1], label="expected photon front", color="red")
