...
/**
 * @brief Parallelize a function over data blocks using the threadpool
 *
 * @param buffer The #particle_buf
 * @param tp The #threadpool to use
 * @param map_function Threadpool mapper function
 * @param extra_data Extra data to pass to the mapper function
 *
 */
void particle_buffer_threadpool_map(struct particle_buffer *buffer,
  ...
  /* Make an array of pointers to the data blocks */
  struct particle_buffer_block **blocks = malloc(sizeof(struct particle_buffer_block *)*nr_blocks);
  nr_blocks = 0;
  block = buffer->first_block;
  while(block) {
  ...
  /* Call threadpool_map on the data blocks */
  threadpool_map(tp, map_function, blocks, nr_blocks,
                 sizeof(struct particle_buffer_block *), 1,
                 extra_data);
