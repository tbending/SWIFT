...
void engine_rebuild(struct engine *e, const int repartitioned,
  ...
  if (e->verbose)
    message(
        "Space has memory for %zd/%zd/%zd/%zd/%zd part/gpart/sink/spart/bpart "
        "(%zd/%zd/%zd/%zd/%zd MB)",
        e->s->size_parts, e->s->size_gparts, e->s->sinks.size_parts,
        e->s->size_sparts, e->s->size_bparts,
        e->s->size_parts * sizeof(struct part) / (1024 * 1024),
        e->s->size_gparts * sizeof(struct gpart) / (1024 * 1024),
        e->s->sinks.size_parts * sizeof(struct sink) / (1024 * 1024),
        e->s->size_sparts * sizeof(struct spart) / (1024 * 1024),
        e->s->size_bparts * sizeof(struct bpart) / (1024 * 1024));

  if (e->verbose)
    message(
        "Space holds %zd/%zd/%zd/%zd/%zd part/gpart/sink/spart/bpart (fracs: "
        "%f/%f/%f/%f/%f)",
        e->s->nr_parts, e->s->nr_gparts, e->s->sinks.nr_parts,
        e->s->nr_sparts, e->s->nr_bparts,
        e->s->nr_parts ? e->s->nr_parts / ((double)e->s->size_parts) : 0.,
        e->s->nr_gparts ? e->s->nr_gparts / ((double)e->s->size_gparts) : 0.,
        e->s->sinks.nr_parts ? e->s->sinks.nr_parts / ((double)e->s->sinks.size_parts) : 0.,
  ...
  long long num_particles[5] = {
      ...
      (long long)(e->s->sinks.nr_parts - e->s->sinks.nr_extra_parts),
 ...
 * multipoles. */
  ...
  e->sinks.updates_since_rebuild = 0;
...
void engine_step(struct engine *e) {
  ...
  if (e->nodeID == 0) {
    ...
    printf(
        ...
        e->sinks.updates, e->s_updates, e->b_updates, e->wallclock_time, e->step_props);
#ifdef SWIFT_DEBUG_CHECKS
    fflush(stdout);
#endif
  ...
  e->sinks.updates_since_rebuild += e->collect_group1.sinks.updated;
...
void engine_recompute_displacement_constraint(struct engine *e) {
  ...
  float min_mass[swift_type_count] = {
      e->s->min_part_mass,  e->s->min_gpart_mass, FLT_MAX, e->s->sinks.min_part_mass,
  ...
  float vel_norm[swift_type_count] = {
      e->s->sum_part_vel_norm,  e->s->sum_gpart_vel_norm, 0.f, e->s->sinks.sum_part_vel_norm,
  ...
  const long long total_nr_baryons =
      e->total_nr_parts + e->total_nr_sparts + e->total_nr_bparts + e->sinks.total_nr_parts;
  ...
  if (N_b > 0.f) {
    ...
    const float min_mass_b = min4(min_mass[0], min_mass[3],
                                  min_mass[4], min_mass[5]);
