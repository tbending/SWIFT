 ...
 *
 * @param e The #engine.
 * @param sp The s-particle.
 * @param buff The return buffer (already allocated thanks to #stars_logger_write_particles).
 */
INLINE static void stars_logger_acceleration_conversion(const struct engine *e,
                                                        const struct spart* sp,
                                                        char *buff) {
  /* Copy it to the buffer */
  memcpy(buff, sp->gpart->a_grav, sizeof(sp->gpart->a_grav));
}
 ...
 */
INLINE static int stars_logger_write_particles(const struct spart* sparts,
                                                struct mask_data* list) {

#ifdef WITH_LOGGER
  /* List what we want to write */
  list[0] = logger_io_make_output_field("Coordinates", sparts, x);

  list[1] = logger_io_make_output_field("Velocities", sparts, v);

  /* Could be done without conversion. Here it is done for the example */
  list[2] = logger_io_make_output_field_conversion("Accelerations", 3 * sizeof(float),
                                                   stars_logger_acceleration_conversion);

  list[3] = logger_io_make_output_field("Masses", sparts, mass);

  list[4] =
    logger_io_make_output_field("ParticleIDs", sparts, id);
