...
__attribute__((always_inline)) INLINE static void rt_check_unphysical_density(
  ...
  if (isnan(*energy) || isinf(*energy))
    error("Got inf/nan radiation energy case%d %.6e | %.6e %.6e %.6e", c, *energy, flux[0], flux[1], flux[2]);
  if (*energy <= 0.f) {
    *energy = 0.f;
    flux[0] = 0.f;
    flux[1] = 0.f;
...
__attribute__((always_inline)) INLINE static void rt_check_unphysical_conserved(
  ...
  if (isinf(*energy) || isnan(*energy))
    error("Got inf/nan radiation energy %.6e | %.6e %.6e %.6e", *energy, flux[0], flux[1], flux[2]);
  if (*energy <= 0.f) {
    *energy = 0.f;
    flux[0] = 0.f;
    flux[1] = 0.f;
