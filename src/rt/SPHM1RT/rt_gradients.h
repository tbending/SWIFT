 ...
 */
__attribute__((always_inline)) INLINE static void radiation_gradient_SPH(const float uini, const float uinj, 
    const float mi, const float mj, const float forcefi, const float forcefj, const float rhoi, const float rhoj,
    const float wi_dr, const float wj_dr, const float *dx, const float r, int diffmode, float *gradi, float *gradj) {
  const float rho_i_inv = 1.0f / rhoi; 
  const float rho_j_inv = 1.0f / rhoj;
  const float r_inv = 1.0f / r;
  if (diffmode == 0){
    const float fradpre_common = (rhoi * uini - rhoj * uinj);
    const float fradprei = mj * forcefi * rho_i_inv * rho_i_inv * fradpre_common  * wi_dr; 
    const float fradprej = mi * forcefj * rho_j_inv * rho_j_inv * fradpre_common  * wj_dr;
    ...
    const float fradpre_common = uinj * rho_j_inv * wj_dr * forcefj 
                                + uini * rho_i_inv * wi_dr * forcefi;
    ...
    const float fradpre_common = (rhoi * uini - rhoj * uinj);
    const float fradprei = mj * rho_i_inv * rho_i_inv * fradpre_common * (wi_dr + wj_dr) * 0.5f ; 
    const float fradprej = mi * rho_j_inv * rho_j_inv * fradpre_common * (wi_dr + wj_dr) * 0.5f ;
    gradi[0] = - fradprei * dx[0] * r_inv;
    gradi[1] = - fradprei * dx[1] * r_inv;
    gradi[2] = - fradprei * dx[2] * r_inv;
    gradj[0] = - fradprej * dx[0] * r_inv;
 ...
 * @param diffmode mode=0 for ``difference''; mode=1 for ``symmetric'' 
 */
__attribute__((always_inline)) INLINE static void radiation_gradient_aniso_SPH(const float uini, const float uinj, 
    const float mi, const float mj, const float forcefi, const float forcefj, const float rhoi, const float rhoj,
    const float wi_dr, const float wj_dr, float Fanisoi[3][3], float Fanisoj[3][3], const float *dx, 
    const float r, int diffmode, float *gradi, float *gradj) {
  const float rho_i_inv = 1.0f / rhoi; 
  const float rho_j_inv = 1.0f / rhoj;
  const float r_inv = 1.0f / r;
  if (diffmode == 0){
    float tempi[3], tempj[3];
    tempi[0] = Fanisoi[0][0] * dx[0] + Fanisoi[0][1] * dx[1] + Fanisoi[0][2] * dx[2]; 
    tempi[0] *= rhoi * uini * r_inv;
    tempi[1] = Fanisoi[1][0] * dx[0] + Fanisoi[1][1] * dx[1] + Fanisoi[1][2] * dx[2]; 
    tempi[1] *= rhoi * uini * r_inv;
    tempi[2] = Fanisoi[2][0] * dx[0] + Fanisoi[2][1] * dx[1] + Fanisoi[2][2] * dx[2]; 
    tempi[2] *= rhoi * uini * r_inv;
    tempj[0] = Fanisoj[0][0] * dx[0] + Fanisoj[0][1] * dx[1] + Fanisoj[0][2] * dx[2];  
    tempj[0] *= rhoj * uinj * r_inv;
    tempj[1] = Fanisoj[1][0] * dx[0] + Fanisoj[1][1] * dx[1] + Fanisoj[1][2] * dx[2];  
    tempj[1] *= rhoj * uinj * r_inv;
    tempj[2] = Fanisoj[2][0] * dx[0] + Fanisoj[2][1] * dx[1] + Fanisoj[2][2] * dx[2];  
    tempj[2] *= rhoj * uinj * r_inv;
    gradi[0] = -(tempi[0] - tempj[0]) * mj * rho_i_inv * rho_i_inv * forcefi * wi_dr;
    gradi[1] = -(tempi[1] - tempj[1]) * mj * rho_i_inv * rho_i_inv * forcefi * wi_dr;
    gradi[2] = -(tempi[2] - tempj[2]) * mj * rho_i_inv * rho_i_inv * forcefi * wi_dr;
    gradj[0] = -(tempi[0] - tempj[0]) * mi * rho_j_inv * rho_j_inv * forcefj * wj_dr;
    gradj[1] = -(tempi[1] - tempj[1]) * mi * rho_j_inv * rho_j_inv * forcefj * wj_dr;
    gradj[2] = -(tempi[2] - tempj[2]) * mi * rho_j_inv * rho_j_inv * forcefj * wj_dr;      
  } else if (diffmode == 1){
    float tempi[3], tempj[3];
    tempi[0] = Fanisoi[0][0] * dx[0] + Fanisoi[0][1] * dx[1] + Fanisoi[0][2] * dx[2]; 
    ...
    tempj[2] = Fanisoj[2][0] * dx[0] + Fanisoj[2][1] * dx[1] + Fanisoj[2][2] * dx[2];  
    tempj[2] *= rhoj * uinj * r_inv;
    gradi[0] = -(tempi[0] - tempj[0]) * mj * rho_i_inv * rho_i_inv * (wi_dr + wj_dr) * 0.5f ;
    gradi[1] = -(tempi[1] - tempj[1]) * mj * rho_i_inv * rho_i_inv * (wi_dr + wj_dr) * 0.5f ;
    gradi[2] = -(tempi[2] - tempj[2]) * mj * rho_i_inv * rho_i_inv * (wi_dr + wj_dr) * 0.5f ;
    gradj[0] = -(tempi[0] - tempj[0]) * mi * rho_j_inv * rho_j_inv * (wi_dr + wj_dr) * 0.5f ;
    gradj[1] = -(tempi[1] - tempj[1]) * mi * rho_j_inv * rho_j_inv * (wi_dr + wj_dr) * 0.5f ;
    gradj[2] = -(tempi[2] - tempj[2]) * mi * rho_j_inv * rho_j_inv * (wi_dr + wj_dr) * 0.5f ; 
  } else if (diffmode == 3){
    float tempi[3], tempj[3];
    tempi[0] = Fanisoi[0][0] * dx[0] + Fanisoi[0][1] * dx[1] + Fanisoi[0][2] * dx[2]; 
 ...
 * @param diffmode mode=0 for ``difference''; mode=1 for ``symmetric'' 
 */
__attribute__((always_inline)) INLINE static void radiation_divergence_SPH(const float *fini, const float *finj, 
    const float mi, const float mj, const float forcefi, const float forcefj, const float rhoi, const float rhoj,
    const float wi_dr, const float wj_dr, const float *dx, const float r, int diffmode, float *divfi, float *divfj) {
  const float rho_i_inv = 1.0f / rhoi; 
  const float rho_j_inv = 1.0f / rhoj;
  const float r_inv = 1.0f / r;
  if (diffmode == 0){
    ...
    const float drhofdr = drhof[0] * dx[0] + drhof[1] * dx[1] + drhof[2] * dx[2];
 ...
 * @param diffmode mode=0 for ``difference''; mode=1 for ``symmetric'' 
 */
__attribute__((always_inline)) INLINE static void radiation_divergence_aniso_SPH(const float *fini, const float *finj, 
    const float mi, const float mj, const float forcefi, const float forcefj, const float rhoi, const float rhoj,
    const float wi_dr, const float wj_dr, float Fanisoi[3][3], float Fanisoj[3][3], const float *dx, 
  ...
  if (diffmode == 0){
    float tempi, tempj;
    tempi  = (Fanisoi[0][0] * dx[0] + Fanisoi[0][1] * dx[1] + Fanisoi[0][2] * dx[2]) * fini[0]; 
    tempi += (Fanisoi[1][0] * dx[0] + Fanisoi[1][1] * dx[1] + Fanisoi[1][2] * dx[2]) * fini[1];
    tempi += (Fanisoi[2][0] * dx[0] + Fanisoi[2][1] * dx[1] + Fanisoi[2][2] * dx[2]) * fini[2]; 
    tempi *= rhoi * r_inv;
    tempj  = (Fanisoj[0][0] * dx[0] + Fanisoj[0][1] * dx[1] + Fanisoj[0][2] * dx[2]) * finj[0]; 
    tempj += (Fanisoj[1][0] * dx[0] + Fanisoj[1][1] * dx[1] + Fanisoj[1][2] * dx[2]) * finj[1]; 
    tempj += (Fanisoj[2][0] * dx[0] + Fanisoj[2][1] * dx[1] + Fanisoj[2][2] * dx[2]) * finj[2]; 
    tempj *= rhoj * r_inv; 
    ...
    *divfj = - (tempi + tempj) * mi;   
    ...
    tempj += (Fanisoj[2][0] * dx[0] + Fanisoj[2][1] * dx[1] + Fanisoj[2][2] * dx[2]) * finj[2]; 
    tempj *= rhoj * r_inv; 
    *divfi = -(tempi - tempj) * mj * rho_i_inv * rho_i_inv * (wi_dr + wj_dr) * 0.5f ;
    *divfj = -(tempi - tempj) * mi * rho_j_inv * rho_j_inv * (wi_dr + wj_dr) * 0.5f ; 
  } else {
    error("diffmode should be 0 or 1 or 2");
  }
}
 ...
 * @param diffmode mode=0 for ``difference''; mode=1 for ``symmetric'' 
 */
__attribute__((always_inline)) INLINE static void radiation_gradient_vec_SPH(const float *fini, const float *finj, 
    const float mi, const float mj, const float forcefi, const float forcefj, const float rhoi, const float rhoj,
    const float wi_dr, const float wj_dr, const float *dx, const float r, int diffmode, float shearfi[3][3], float shearfj[3][3]) {
  const float rho_i_inv = 1.0f / rhoi; 
  const float rho_j_inv = 1.0f / rhoj;
  const float r_inv = 1.0f / r;
  if (diffmode == 0){
