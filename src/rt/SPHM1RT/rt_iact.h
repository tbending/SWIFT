...
__attribute__((always_inline)) INLINE static void radiation_force_loop_function(
  ...
  for (int g = 0; g < RT_NGROUPS; g++) {
    ...
    foxi = max(expf(-rpi->params.chi[g] * rhoi * hi),
               fradmagi / (credi + FLT_MIN) / (uradi + FLT_MIN));
    foxj = max(expf(-rpj->params.chi[g] * rhoj * hj),
               fradmagj / (credj + FLT_MIN) / (uradj + FLT_MIN));
    ...
    fdrhati = min(fabsf((fraduniti[0] * dx[0] + fraduniti[1] * dx[1] +
                         fraduniti[2] * dx[2]) *
                        r_inv),
                  1.0f);
    fdrhatj = min(fabsf((fradunitj[0] * dx[0] + fradunitj[1] * dx[1] +
                         fradunitj[2] * dx[2]) *
                        r_inv),
                  1.0f);
    ...
    /* slope limiter */
    rhoucmid = (rhoi * uradi * credi * hi + rhoj * uradj * credj * hj) /
               (hi + hj + FLT_MIN);
    ratioflux = fabsf(rhoucmid - rhoj * uradj * credj - rhoucdrj + FLT_MIN);
    ratioflux = fabsf(rhoi * uradi * credi + rhoucdri - rhoucmid) /
                (ratioflux + FLT_MIN);
    ...
    diss_durad_term = 1.f / rhomean2 * (ddi + ddj) *
