 ...
 *
 */
__attribute__((always_inline)) INLINE static void radiation_gradient_loop_function(
    ...
    float fradmfi[3*RT_NGROUPS];
    float fradmfj[3*RT_NGROUPS];
    ...
    radiation_get_comoving_radiation_energy_multifrequency(pi, uradmfi);
    radiation_get_comoving_radiation_energy_multifrequency(pj, uradmfj);

    radiation_get_comoving_radiation_flux_multifrequency(pi, fradmfi);
    radiation_get_comoving_radiation_flux_multifrequency(pj, fradmfj);
    ...
    for (int g = 0; g < RT_NGROUPS; g++) {
        ...
        rhoi, rhoj, wi_dr, wj_dr, dx, r, diffmode, gradi, gradj);
...
__attribute__((always_inline)) INLINE static void radiation_force_loop_function(
    ...
    float divfipar, divfjpar; /* divfipar is inside the loop, and divfi is summed over particle */
    float divfi, divfj;
    ...
    for (int g = 0; g < RT_NGROUPS; g++) { 
        ...
        funiti[2] = fradi[2] / (fradmagi + FLT_MIN);
        funitj[0] = fradj[0] / (fradmagj + FLT_MIN);
        ...
        foxj = max(expf(-rpj->params.chi[g] * rhoj * hj) , fradmagj / (credj + FLT_MIN) / (uradj + FLT_MIN));
        ...
        sqi = max(FLT_MIN,4.f - 3.f * foxi * foxi);
        sqj = max(FLT_MIN,4.f - 3.f * foxj * foxj);
        ...
        fradunitj[0] = fradj[0] / (fradmagj + FLT_MIN);
        fradunitj[1] = fradj[1] / (fradmagj + FLT_MIN);
        fradunitj[2] = fradj[2] / (fradmagj + FLT_MIN);

        fdrhati = min(fabsf((fraduniti[0] * dx[0] +
                                      fraduniti[1] * dx[1] +
                                      fraduniti[2] * dx[2])
                                      * r_inv),
                                1.0f);
        fdrhatj = min(fabsf((fradunitj[0] * dx[0] +
                                      fradunitj[1] * dx[1] +
                                      fradunitj[2] * dx[2])
                                      * r_inv),
                                1.0f); 
        ...
        /* (rj-ri) dot grad u */
        rhoucdri = -rhoi *  (graduci[0] * dx[0] 
                            + graduci[1] * dx[1] 
                            + graduci[2] * dx[2]) 
                            * hi / (hi + hj);
        rhoucdrj = rhoj *  (graducj[0] * dx[0] 
                            + graducj[1] * dx[1] 
                            + graducj[2] * dx[2]) 
                            * hj / (hi + hj);                              
        drhouc_high = rhoucdri - rhoucdrj;
        /* slope limiter */
        rhoucmid = (rhoi * uradi * credi * hi + rhoj * uradj * credj * hj) / (hi + hj + FLT_MIN);
        ratioflux = fabsf(rhoucmid - rhoj * uradj * credj - rhoucdrj + FLT_MIN);
        ratioflux = fabsf(rhoi * uradi * credi + rhoucdri - rhoucmid) / (ratioflux + FLT_MIN);
        slopelimiter = min(1.0f, ratioflux);
        slopelimiter = max(0.0f, slopelimiter);
        rhomean2 = min(rhoi,rhoj) *  min(rhoi,rhoj);
        if (((rhoi * uradi * credi > 0.5f * rhoj * uradj * credj)||(rhoi * uradi * credi < 0.5f * rhoj * uradj * credj))
        ...
        diss_durad_term =
          1.f / rhomean2 * (ddi + ddj) 
          * (drhou_low + drhouc_high * slopelimiter / cred0) 
          * (wi_dr_temp + wj_dr_temp) * 0.5f * r_inv;
        ...
        radiation_gradient_aniso_SPH(ddfi * divfi, ddfj * divfj, mi, mj,
        pi->force.f, pj->force.f, rhoi, rhoj, wi_dr, wj_dr, J_tensori, J_tensorj, 
        dx, r, diffmodeaniso, diss_dfrad_term_i, diss_dfrad_term_j);
        ...
        rpi->cdt[g].energy += mj * durad_dt_i * cred0 / credi;
        rpi->cdt[g].flux[0] += mj * dfrad_dt_i[0];
        rpi->cdt[g].flux[1] += mj * dfrad_dt_i[1];
        rpi->cdt[g].flux[2] += mj * dfrad_dt_i[2];
        if (mode==1) {
          rpj->cdt[g].energy += mi * durad_dt_j * cred0 / credj; 
          rpj->cdt[g].flux[0] += mi * dfrad_dt_j[0];
          rpj->cdt[g].flux[1] += mi * dfrad_dt_j[1];
          rpj->cdt[g].flux[2] += mi * dfrad_dt_j[2];
        }
