...
__attribute__((always_inline)) INLINE static void lightcone_check_particle_crosses(
  ...
  for(int lightcone_nr=0; lightcone_nr<nr_lightcones; lightcone_nr+=1) {
    ...
    struct lightcone_props *props = e->lightcone_array_properties->lightcone+lightcone_nr;
    struct replication_list *replication_list = replication_list_array+lightcone_nr;
