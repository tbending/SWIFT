...
if test "$ac_test_CXXFLAGS" != "set"; then
  case $ax_cv_cxx_compiler_vendor in
    dec) CXXFLAGS="$CXXFLAGS -newc -w0 -O5 -ansi_alias -ansi_args -fp_reorder -tune host"
	 if test "x$acx_maxopt_portable" = xno; then
           CXXFLAGS="$CXXFLAGS -arch host"
         fi;;

    sun) CXXFLAGS="$CXXFLAGS -native -fast -xO5 -dalign"
	 if test "x$acx_maxopt_portable" = xyes; then
	   CXXFLAGS="$CXXFLAGS -xarch=generic"
         fi;;

    hp)  CXXFLAGS="$CXXFLAGS +Oall +Optrs_ansi +DSnative"
	 if test "x$acx_maxopt_portable" = xyes; then
    ...
    ibm) if test "x$acx_maxopt_portable" = xno; then
		...
		CXXFLAGS="$CXXFLAGS -O3 -qansialias -w $xlc_opt",
               [CXXFLAGS="$CXXFLAGS -O3 -qansialias -w"
                echo "******************************************************"
    ...
    intel) CXXFLAGS="$CXXFLAGS -O3 -ansi-alias"
	if test "x$acx_maxopt_portable" = xno; then
                  ...
                  case $ax_cv_gcc_x86_cpuid_1 in
                    ...
                    *070?f??:*:*:*|70?f??:*:*:*) icc_flags="-march=core-avx2" ;;
                                   83?f??:*:*:*) icc_flags="-march=core-avx2"
                                                 CXXFLAGS="$CXXFLAGS -fma -ftz -fomit-frame-pointer";; # EPYC
                  esac ;;
              esac ;;
          esac
    ...
    clang)
     ...
     AX_CHECK_COMPILE_FLAG(-fstrict-aliasing, CXXFLAGS="$CXXFLAGS -fstrict-aliasing")
